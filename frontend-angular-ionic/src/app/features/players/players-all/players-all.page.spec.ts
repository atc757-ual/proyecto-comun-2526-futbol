...
describe('PlayersAllPage', () => {
  ...
  it('should filter players by team', () => {
    ...
    expect(c.filteredPlayers().length).toBe(1);
    expect(c.filteredPlayers()[0].name).toBe('Ronaldo');
  });
  ...
  it('should return all players when search term is empty', () => {
    ...
    expect(c.filteredPlayers().length).toBe(3);
  });
  ...
  it('should not exceed total pages', () => {
    c.itemsPerPage.set(10);
    component.nextPage();
    expect(c.currentPage()).toBe(1); // solo 3 players, 1 page
  });

  it('should go to specific page', () => {
    c.itemsPerPage.set(1);
  ...
  it('should detect own players with isMine', () => {
    const myPlayer = mockPlayers[0];
    const otherPlayer = mockPlayers[1];
